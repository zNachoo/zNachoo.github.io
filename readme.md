...
<head>
    ...
    <style>
        ...
        body {
            ...
            min-height: 100vh;
            margin: 0;
            overflow: hidden; /* Evita barras de desplazamiento cuando el botón "No" se mueve */
        }
        ...
        .card {
            ...
            width: 90%;
            text-align: center;
            z-index: 10; /* Asegura que la tarjeta esté encima del botón de "No" */
        }

        /* Estilo para el botón "No" que se va a mover y reducir */
        #btnNo {
            position: absolute;
            /* Aplicamos transición suave para el movimiento y el tamaño (transform) */
            transition: transform 0.2s ease-out, background-color 0.1s; 
            z-index: 5;
            padding: 0.75rem 1.5rem;
            border-radius: 9999px; /* Botón súper redondeado */
            font-weight: 600;
            color: white;
        ...
        #btnNo:hover {
            background-color: #e53935; /* Un poco más oscuro al pasar el ratón */
        }

        /* Oculta el botón cuando se acepta el "Sí" */
        #btnNo.hidden {
            display: none;
        }

        /* Estilo para el mensaje final de respuesta "Sí" */
        .respuesta-si {
...
<body>
    ...
    <div id="gameContainer" class="flex flex-col items-center justify-center w-full h-full">
        ...
        <div id="questionCard" class="card">
            ...
            <p id="answerMessage" class="mt-8 respuesta-si hidden">
                <!-- Aquí irá el mensaje de "yo también te amo" -->
            </p>
        </div>

        <!-- Botón "No" separado para que pueda ser posicionado absolutamente -->
        <!-- La función moveNoButton() se llama en cada clic -->
    ...
    <script>
        ...
        function moveNoButton() {
            ...
            // questionText.textContent = '¿Me amas?'; // Ya está configurado así al inicio
        }

        // 7. Inicializar el botón "No" en una posición fija (al lado del "Sí") al cargar la página
        window.onload = () => {
